...
* **Responsive Collapsible Sidebar**:
  ...
  * Organizes navigation into collapsible domain groups: *Academic*, *Attendance*, *Examinations*, *Finance*, *Communication*, *Administration*, and *Account Settings* (including Multi-Device Sessions).
    ...
    * **Attendance & Leave Domain (Phase 6)**: Includes *Take Attendance* (`/erp/attendance/mark` — interactive `<AttendanceSheet />` consuming `PUBLISHED` timetable slots and student enrollments with one-click present toggle and lock status check), *Attendance Register* (`/erp/attendance/register` — tabular daily, weekly, monthly, and yearly matrix), *Bulk Attendance* (`/erp/attendance/bulk` — multi-section/period batch marker), *Leave Requests* (`/erp/attendance/leaves` — student & teacher leave application and review workflows), *Attendance Corrections* (`/erp/attendance/corrections` — teacher correction requests with reason and admin review), and *Attendance Dashboard & Analytics* (`/erp/attendance/analytics` — monthly and session attendance percentage and defaulter monitoring).
