...
* **Responsive Collapsible Sidebar**:
  ...
  * Organizes navigation into collapsible domain groups: *Academic*, *Attendance*, *Examinations*, *Finance*, *Communication*, *Administration*, and *Account Settings* (including Multi-Device Sessions).
    ...
    * **Transport, Fleet & GPS Tracking Domain (Phase 13)**: Includes *Transport Dashboard* (`/erp/transport/dashboard` — unified control center showing active fleet status, daily route progress, alert cards for expiring insurance/fitness/licenses, and quick KPI stats), *Vehicle Management* (`/erp/transport/vehicles` — full CRUD data table and modal forms for fleet vehicles, tracking capacity, insurance, fitness certificates, and maintenance status), *Driver Management* (`/erp/transport/drivers` — roster and profile cards for drivers with license verification, medical expiry tracking, and emergency contact details), *Route Builder* (`/erp/transport/routes` — interactive route builder interface with source/destination configuration, sequence ordering of stops, and duration/distance calculation), *Stop Management* (`/erp/transport/stops` — master directory of bus stops with pickup/drop timetables, GPS coordinate mapping, and assigned student count indicators), *Assignment Management* (`/erp/transport/assignments` — student-to-transport allocation interface with capacity warning badges, route/stop filter selectors, and one-click enrollment assignment), *GPS Dashboard* (`/erp/transport/gps` — live fleet tracking monitor showing last-known coordinates, speed, route progress, off-route warnings, and ETA for each vehicle), *Maintenance Dashboard* (`/erp/transport/maintenance` — service log and cost tracker for repairs, fuel logs, scheduled servicing, and insurance/fitness renewals), and *Transport Analytics* (`/erp/transport/analytics` — deep-dive analytical charts for vehicle utilization %, route occupancy, cost per km, and fleet efficiency).
